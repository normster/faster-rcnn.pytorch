...
def get_boxes_labels(model, im_data, im_info, gt_boxes, num_boxes):
    ...
    #print(cls_scores.size())
    # based on eqn. (2) in the paper, we need to give it a target class.
    adv_labels = permute_labels(gt_labels) # here, we want to attack the class as the following class.
    adv_scores = cls_scores[range(adv_labels.size(0)), adv_labels]
    #print(adv_scores.size())

    return adv_scores, adv_labels, pred_scores, pred_labels, gt_scores, gt_labels, False
...
def run_attack(model, data, steps=150, gamma=0.5):
    ...
    for i in range(steps):
        ...
        adv_scores, adv_labels, pred_scores, pred_labels, gt_scores, gt_labels, _flag = get_boxes_labels(model, im_data + eps, im_info, gt_boxes, num_boxes)
        #adv_labels = permute_labels(gt_labels)
        if _flag == True:
            print('There is no good proposals %d iters' % i)
            break
        if (adv_labels == pred_labels).all():
        #if torch.sum(pred_labels == gt_labels) < gt_labels.size(0)//2:
            print('Successful before %d iters' % i)
            break
...
if __name__ == '__main__':
  ...
  for i in range(attack_total):
      ...
      if vis:
          cv2.imwrite('adv_image/result'+str(i)+'.png', im2show)
          #pdb.set_trace()
